...
//#region node_modules/.nitro/vite/services/ssr/assets/chat-BnVElfcb.js
...
function buildSystemPrompt(companyName) {
	return `You are the "Company Intelligence AI", a specialized corporate analyst chatbot embedded within the KITS Placement Hub platform. Your knowledge base is derived from a comprehensive companies_master dataset with 100+ data points per company. The user is currently viewing: **${companyName}**. Data dimensions: Firmographics, Financials & Growth (Revenue, Valuation, YoY Growth, Funding/Investors), Culture & HR (Work Culture, Leave Policy, Health Support, ESOPs, Burnout Risk, Layoff History, Hiring Velocity), Technology (AI/ML Adoption, Tech Stack, Cloud Partners, Cybersecurity), Operations (Remote Policy, Working Hours, Transport Policy), Competitors & Strategy (Key Competitors, Competitive Advantages, Future Projections, Weaknesses). Rules: Base answers ONLY on the dataset. Acknowledge unavailable (NA) data honestly. Use bullet points and **bold** for key metrics. Be direct and analytical — no filler. CRITICAL: At the very end of your response, ALWAYS provide exactly 3 suggested follow-up questions that the user could ask next. Format these follow-up questions EXACTLY like this (with the triple dash):\n\n---\nSuggested questions:\n1. [Question 1]\n2. [Question 2]\n3. [Question 3]`;
}
var MAPPINGS = [
	{
		keywords: [
	...
	},
	{
		...
		fields: [
			...
			},
			{
				...
				label: "Average Retention / Tenure"
			},
			{
				key: "burnout_risk",
				label: "Burnout Risk"
...
async function loadCompanies() {
	...
	try {
		...
		if (fs.existsSync(csvPath)) csvText = fs.readFileSync(csvPath, "utf-8");
		else {
			const altPath = path.join(process.cwd(), "data", "companies_master(Master Data).csv");
			if (fs.existsSync(altPath)) csvText = fs.readFileSync(altPath, "utf-8");
			else throw new Error(`CSV file not found at ${csvPath} or ${altPath}`);
		}
		...
		for (let i = 1; i < lines.length; i++) {
			...
			headers.forEach((header, index) => {
				let val = values[index] !== void 0 ? values[index] : "";
...
async function findCompany(companyName) {
	...
	let found = companies.find((c) => c.short_name?.toLowerCase() === searchName || c.name?.toLowerCase() === searchName);
	if (!found) found = companies.find((c) => c.short_name && c.short_name.toLowerCase().includes(searchName) || c.name && c.name.toLowerCase().includes(searchName) || searchName.includes(c.short_name?.toLowerCase() || "") || searchName.includes(c.name?.toLowerCase() || ""));
	return found || null;
}
function buildDefaultSummary(company) {
	const getVal = (key, label) => {
		const val = company[key];
		if (!val || val.toUpperCase() === "NA" || val.toUpperCase() === "NOT APPLICABLE" || val.toLowerCase() === "n/a") return "Not specified";
		return val;
	};
	return `### 📋 **${company.name || "Company"}** (${company.category || "Enterprise"}) — Overview\n\n**Overview**: ${getVal("overview_text", "Company Overview")}\n\n#### ⚙️ Quick Profile\n* **Headquarters**: ${getVal("headquarters_address", "HQ Address")}\n* **Founded**: ${getVal("incorporation_year", "Founded")}\n* **Employee Size**: ${getVal("employee_size", "Employee Size")}\n* **Remote Policy**: ${getVal("remote_policy_details", "Remote Policy")}\n* **Glassdoor Rating**: ⭐ **${getVal("glassdoor_rating", "Glassdoor Rating")}**\n* **Annual Revenue**: **${getVal("annual_revenue", "Annual Revenue")}**\n* **Valuation**: **${getVal("valuation", "Valuation")}**\n\nYou can ask me specific questions about **Culture**, **Tech Stack**, **Salary & Benefits**, **Competitors**, **Leadership**, **Layoffs**, or **Career Growth**.`;
}
function generateLocalReply(company, query) {
	...
	const matches = [];
	for (const m of MAPPINGS) if (m.keywords.some((keyword) => q.includes(keyword))) matches.push(m);
	if (matches.length === 0) return buildDefaultSummary(company);
	let response = "";
	for (const match of matches) {
		let sectionContent = "";
		for (const field of match.fields) {
			const rawVal = company[field.key];
			if (!rawVal || rawVal.toUpperCase() === "NA" || rawVal.toUpperCase() === "NOT APPLICABLE" || rawVal.toLowerCase() === "n/a") continue;
			if (field.format === "list") sectionContent += `* **${field.label}**:\n${formatList(rawVal)}\n`;
			else if (field.format === "url") sectionContent += `* **${field.label}**: [Link / Profile](${rawVal})\n`;
			else sectionContent += `* **${field.label}**: ${rawVal}\n`;
		}
		if (sectionContent) response += `### ${match.emoji} ${match.title} for **${company.short_name || company.name}**\n\n` + sectionContent + "\n";
	}
	if (response.trim() === "") response = buildDefaultSummary(company);
	const selected = [
...
var chatWithGemini_createServerFn_handler = createServerRpc({
	id: "57dd3b36b2cdcfcbbaf3ea3ef81c88448fb18c571c86ac5b208919ce183ef769",
	name: "chatWithGemini",
	filename: "src/routes/api/chat.ts"
...
var chatWithGemini = createServerFn({ method: "POST" }).validator((data) => {
...
}).handler(chatWithGemini_createServerFn_handler, async ({ data }) => {
	...
	if (isRealKey) {
		...
		try {
			...
			if (raw.ok) {
				...
				if (result.error) geminiError = `Gemini API error (${result.error.code}): ${result.error.message}`;
				else {
	...
	} else geminiError = isRealKey ? "GEMINI_API_KEY validation failed." : "GEMINI_API_KEY is not configured or is a placeholder.";
	...
	if (!company) return { reply: `🤖 I couldn't find detailed records for **${companyName}**. Please try again or check your query.` };
	return { reply: generateLocalReply(company, message) };
});
//#endregion
